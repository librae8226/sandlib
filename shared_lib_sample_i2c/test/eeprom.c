...
#define DEFAULT_NUM_PAGES    4	/* we default to a 24C08 eeprom which has 4 pages */
...
int eeprom_write(int fd, unsigned int addr, unsigned int offset,
		 unsigned char *buf, unsigned char len)
{
	...
	if (len + offset > 256) {
		fprintf(stderr,
			"Sorry, len(%d)+offset(%d) > 256 (page boundary)\n",
			len, offset);
		return -1;
	}
...
int eeprom_read(int fd, unsigned int addr, unsigned int offset,
		unsigned char *buf, unsigned char len)
{
	struct i2c_msg i2cmsg;
	int i;
	...
	fprintf(stdout, "%08x: \n", offset);
	for (i = 0; i < len; i++)
		fprintf(stdout, "%02x ", i2cmsg.buf[i]);
	fprintf(stdout, "\n");
#endif
	return 0;
}

static void command_help(char *cmd)
{
...
int main(int argc, char **argv)
{
	...
	while ((i = getopt(argc, argv, "d:a:p:wyf:h")) >= 0) {
		...
		case 'd':
			...
			if (sscanf(optarg, "%d", &id) != 1) {
				fprintf(stderr,
					"Cannot parse '%s' as id, example: 0\n",
					optarg);
				exit(1);
			}
			break;
	...
	fprintf(stdout, "base-address of eeproms       : 0x%02x\n", addr);
	fprintf(stdout,
		"number of pages to read       : %d (0x%02x .. 0x%02x)\n",
	...
	if (fn) {
		...
		if (f < 0) {
			fprintf(stderr,
				"Could not open data-file %s for reading or writing\n",
				fn);
			perror(fn);
			exit(1);
		}
	...
	fprintf(stdout, "i2c-devicenode is             : %d\n", id);
	fprintf(stdout, "            on filedescriptor : %d\n\n", fd);
	...
	for (i = 0; i < pages; i++) {
		...
		if (rwmode) {
			...
			for (j = 0; j < (BYTES_PER_PAGE / MAX_BYTES); j++)
				if (eeprom_write(fd, addr + i, j * MAX_BYTES,
						 buf + (j * MAX_BYTES), MAX_BYTES) < 0)
					exit(1);
		} else {
			for (j = 0; j < (BYTES_PER_PAGE / MAX_BYTES); j++)
				if (eeprom_read(fd, addr + i, j * MAX_BYTES,
						buf + (j * MAX_BYTES), MAX_BYTES) < 0)
					exit(1);
		}
