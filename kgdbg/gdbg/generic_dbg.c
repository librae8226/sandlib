...
int gdbg_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_WRITE_REG:
		if (arg != 0x0)
			memcpy((volatile unsigned int *)arg, gdbg_buffer, sizeof(unsigned int));
		else
			printk(KERN_ERR "%s: invalid address\n", __FUNCTION__);
		break;
	case CMD_READ_REG:
		memcpy(gdbg_buffer, (volatile unsigned int *)arg, sizeof(unsigned int));
		break;
	default:
...
static int gdbg_init(void)
{
	...
	printk(KERN_INFO "0xF7FCD040: 0x%08X\n", *(unsigned int *)0xF7FCD040);
	printk(KERN_INFO "0xF7EA0000: 0x%08X\n", *(unsigned int *)0xF7EA0000);
	printk(KERN_INFO "0xF7EA0004: 0x%08X\n", *(unsigned int *)0xF7EA0004);
